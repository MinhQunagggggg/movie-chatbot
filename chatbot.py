...
def generate_answer(user_input: str) -> str:
    ...
    phim = fuzzy_find_movie_name(movie_candidate, movies)
    ...
    if intent == "genre_query":
        genres = [
            ...
            "khoa học viễn tưởng", "hoạt hình", "chiến tranh", "hình sự",
            "viễn tây", "âm nhạc", "thể thao", "tài liệu", "gia đình",
            "lịch sử", "giả tưởng", "siêu anh hùng", "anime", "phim ngắn",
        ]
        ...
        if genre_matched:
            ...
            if genre_movies:
                return (
                    f"Các phim thể loại phim {genre_matched.title()} hôm nay:\n"
                    + "\n".join(f"🎬 {m}" for m in genre_movies)
                )
            else:
                return (
                    f"❌ Không có phim thể loại {genre_matched.title()} "
                    f"đang chiếu hôm nay."
                )

    # Tìm theo diễn viên (khi chưa xác định phim cụ thể)
    if intent == "actor" and not phim:
        act_matches = [
            m for m in movies
            if any(raw_name in actor for actor in m["actors_clean"])
        ]
        if act_matches:
            phim_names = ", ".join(sorted({m["movie_name_vn"] for m in act_matches}))
    ...
    # Tìm theo đạo diễn (khi chưa xác định phim cụ thể)
    if intent == "director" and not phim:
        dir_matches = [m for m in movies if raw_name in m["dir_clean"]]
        if dir_matches:
            phim_names = ", ".join(sorted({m["movie_name_vn"] for m in dir_matches}))
            return f"{dir_matches[0]['director']} Đạo diễn phim: {phim_names}"
        else:
            return f"Tôi không tìm thấy đạo diễn nào tên {raw_name.title()}."

    # Người dùng nhắc đến tên phim nhưng chưa rõ muốn hỏi gì
    if intent is None and phim:
        return (
            f"Bạn đang hỏi về phim {phim['movie_name_vn']}. "
            f"Vui lòng nói rõ bạn muốn biết gì: nội dung, trailer, diễn viên...?"
        )

    # Hỏi danh sách phim hôm nay
    if intent == "today_movies":
        movies_today = get_movies_showing_today()
        return (
            "Hôm nay đang chiếu các phim:\n"
            + "\n".join(f"🎞️ {m}" for m in movies_today)
            if movies_today
            else "Không có phim nào đang chiếu hôm nay."
        )

    # Người dùng hỏi "suất chiếu" trong câu tự do
    if not intent and phim and "suat chieu" in user_clean:
        return get_nearest_showtime_for(phim["movie_name_vn"])

    # Suất chiếu gần nhất (nhiều phim hoặc 1 phim)
    if intent == "nearest_showtime_multiple":
        return (
            "\n".join(get_nearest_showtime_for(phim["movie_name_vn"]))
            if phim
            else get_multiple_nearest_showtimes()
        )

    if intent == "nearest_showtime_for_movie":
        return (
            get_nearest_showtime_for(phim["movie_name_vn"])
            if phim
            else "Bạn muốn xem suất chiếu gần nhất của phim nào?"
        )

    # Các intent cụ thể khi đã nhận diện đúng phim
    if phim:
        if intent == "trailer":
            return (
                f"https://youtu.be/{phim['trailer_id']}"
                if phim.get("trailer_id")
                else f"Phim {phim['movie_name_vn']} chưa có trailer."
            )
        elif intent == "content":
            ...
            return (
                f"Phim {phim['movie_name_vn']} được đánh giá "
                f"{phim.get('rating', 'chưa có')} điểm."
            )
